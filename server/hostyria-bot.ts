...
async function sendDashboard(chatId: number) {
  ...
  const keyboard = {
    inline_keyboard: [
      ...
      [{ text: 'Support 🆘', callback_data: 'support' }],
      [{ text: 'Recharge Balance', callback_data: 'recharge_balance' }],
...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'login') {
    ...
    sendWelcomeMessage(chatId);
  } else if (data === 'recharge_balance') {
    bot.sendMessage(chatId, 'Recharge balance feature coming soon. Please contact support for now.');
